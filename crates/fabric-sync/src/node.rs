...
use fabric_config::sync::config::PolicyRules;
...
pub use fabric_config::sync::content_hash;
...
impl SyncNode {
    ...
    fn peer_path_in_scope(&self, path: &str) -> bool {
        match &self.include {
            None => true,
            Some(globs) => fabric_config::sync::glob::matches_any(globs, path),
        }
    }
...
mod tests {
    ...
    fn bus() -> PolicyRules {
        fabric_config::sync::config::SyncPolicy::Bus.rules()
    }
