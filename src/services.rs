...
//! What a service does with that stream lives in its own crate, which depends on
//! `fabric-service-api` and on nothing in this one.
...
pub fn builtin(home: &FabricHome) -> Services {
    Services::new()
        .with(fabric_shell::Shell)
        .with(fabric_exec::Exec)
        .with(fabric_send_file::SendFile::new(home.root().join("inbox")))
        .with(fabric_git::Git::default())
}
...
impl Grants for BookGrants {
    ...
    fn shared(&self, name: &str) -> Option<PathBuf> {
        if self.service != fabric_git::SERVICE {
            return None;
        }
...
pub(crate) async fn send_file(
    ...
    let stream = state
        .open_stream(peer, fabric_send_file::SEND_FILE_ALPN)
        .await?;
    fabric_send_file::send_file(stream, name, path).await
}
...
mod tests {
    ...
    fn the_builtin_set_is_the_four_services_in_order() {
        ...
        let (service, protocol) = services.find(fabric_shell::SHELL_ALPN).unwrap();
        assert_eq!((service.name(), protocol.resumable), ("shell", false));
        let (service, protocol) = services.find(fabric_shell::RESUMABLE_SHELL_ALPN).unwrap();
