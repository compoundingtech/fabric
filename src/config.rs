...
pub use fabric_config::FabricHome;

/// A sync entry may name a trusted peer by its local name or its full id.
impl fabric_config::sync::config::KnownPeers for PeerBook {
    fn knows(&self, selector: &str) -> bool {
        self.peers()
            .iter()
            .any(|peer| peer.name.as_deref() == Some(selector) || peer.id.to_string() == selector)
    }
}
...
impl Denied {
    ...
    pub const WIRE_MARKER: &'static str = fabric_service_api::REFUSAL_MARKER;

    /// Did this error come from a peer refusing us, rather than from the
    /// network?
    pub fn is_refusal(error: &str) -> bool {
        fabric_service_api::is_refusal(error)
    }
}
...
mod tests {
    ...
    #[test]
    fn a_sync_selector_names_a_trusted_peer_by_name_or_full_id() {
        use fabric_config::sync::config::KnownPeers;
        let mut peers = PeerBook::default();
        let id = SecretKey::generate().public();
        peers.add(id, Some("desk".into()), None);
        assert!(peers.knows("desk"));
        assert!(peers.knows(&id.to_string()));
        assert!(!peers.knows("laptop"));
        assert!(
            !peers.knows(&id.to_string()[..8]),
            "a short id is not a selector"
        );
        assert!(!PeerBook::default().knows("desk"));
    }
