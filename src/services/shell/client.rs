...
impl ShellInput {
    ...
    async fn attach(&self, socket: &Path) -> Result<tokio::net::unix::OwnedReadHalf> {
        ...
        super::write_client_resize(&mut write, rows, cols).await?;
    ...
    async fn resize(&self) {
        ...
        if let Some(write) = self.write.lock().await.as_mut() {
            ...
            let _ = super::write_client_resize(write, rows, cols).await;
        }
    }
        ...
        loop {
            ...
            if self.is_forwarding() {
                if let Some(write) = self.write.lock().await.as_mut() {
                    ...
                    let _ = super::write_client_stdin(write, &buf[..read]).await;
                }
                continue;
            }
