...
async fn main() -> Result<()> {
    if fabric_git::invoked_as_remote_helper() {
        let code = fabric_git::run_remote_helper(|| {
            ...
            match command {
                ...
                Commands::Git { command } => match command {
                    GitCommands::InstallHelper => {
                        ...
                        let helper = fabric_git::install_helper_for(&binary)?;
                ...
                Commands::Shell { peer } => {
                    ...
                    let code = fabric_shell::client::run_client(&peer, socket, move || {
                ...
                Commands::Exec { peer, cmd } => {
                    ...
                    let code = fabric_exec::run_client(stream, &peer, &cmd).await?;
                    std::process::exit(code);
                }
...
async fn print_git_status(home: &FabricHome, book: &PeerBook) {
    ...
    match std::env::current_exe() {
        Ok(binary) => {
            let helper = fabric_git::helper_path_for(&binary);
            match (helper, fabric_git::helper_is_installed_for(&binary)) {
